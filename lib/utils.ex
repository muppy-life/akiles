defmodule Akiles.Utils do
  ...
  def reduce_keys_to_atoms({key, val}) when is_map(val), do: {manage_to_existing_atom(key), keys_to_atoms(val)}
  def reduce_keys_to_atoms({key, val}) when is_list(val), do: {manage_to_existing_atom(key), Enum.map(val, &keys_to_atoms(&1))}
